"""Path resolution shared by all actwise components."""
import os
from pathlib import Path

def repo_root(anchor: Path | None = None) -> Path | None:
    """Dev-checkout root: walk up from this file (or anchor) to the first dir
    containing pyproject.toml AND components/. None when wheel-installed."""
    p = (anchor or Path(__file__)).resolve()
    for parent in [p, *p.parents]:
        if (parent / "pyproject.toml").is_file() and (parent / "components").is_dir():
            return parent
    return None

def find_config(filename: str) -> Path:
    """First existing of: $ACTWISE_CONFIG_DIR/<f> -> cwd/<f> -> ~/.actwise/<f> ->
    <dev repo root>/<f>. If none exist, returns ~/.actwise/<f> (a non-existing
    path, so callers' .exists() checks fall through to their built-in defaults,
    and 'config init' style writers have a sane target)."""
    candidates = []
    env_dir = os.environ.get("ACTWISE_CONFIG_DIR")
    if env_dir:
        candidates.append(Path(env_dir).expanduser() / filename)
    candidates.append(Path.cwd() / filename)
    home = Path.home() / ".actwise" / filename
    candidates.append(home)
    root = repo_root()
    if root is not None:
        candidates.append(root / filename)
    for c in candidates:
        if c.exists():
            return c
    return home

def find_config_compat(filename: str, *legacy: str) -> Path:
    """Like find_config, but prefer <filename> and fall back to legacy names.

    Supports the staged actone-data -> actwise-data config rename: resolve the
    new ``actwise-data.yaml`` if it exists anywhere on the search path, otherwise
    fall back to an existing legacy ``actone-data.yaml``. When neither exists,
    return the new-name path (so writers target the new name)."""
    primary = find_config(filename)
    if primary.exists():
        return primary
    for name in legacy:
        legacy_path = find_config(name)
        if legacy_path.exists():
            return legacy_path
    return primary

def getenv_compat(name: str, *legacy: str, default: str | None = None) -> str | None:
    """os.environ.get with back-compat aliases: return the first env var that is
    set among (name, *legacy), else default. Supports the staged ACTONE_DATA_* ->
    ACTWISE_DATA_* env rename without breaking existing deployments."""
    for key in (name, *legacy):
        value = os.environ.get(key)
        if value is not None:
            return value
    return default
