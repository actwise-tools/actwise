...
_MULTIPART_OPERATIONS = {"saveStepChanges"}
...
def invoke(registry, client, op_id, params=None, allow_write=False):
    ...
    params = dict(params or {})
    if op_id == "saveStepChanges":
        identifiers = params.get("workItemIdentifiers")
        if isinstance(identifiers, str):
            try:
                parsed = json.loads(identifiers)
            except json.JSONDecodeError:
                parsed = None
            params["workItemIdentifiers"] = (
                parsed if isinstance(parsed, list) else [identifiers]
            )
        ...
        if isinstance(note, str):
            try:
                ...
                parsed = None
            params["note"] = parsed if isinstance(parsed, dict) else {"note": note}
    ...
    if op_id in _MULTIPART_OPERATIONS:
        request_kwargs["multipart"] = True
    result = client.request(method, path, **request_kwargs)
...
def make_client(base_url=None, user=None, password=None, env=None):
    ...
    client.environment = env or "default"
    return client
