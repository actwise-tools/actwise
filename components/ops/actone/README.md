...
| `actone ops` | `actone.registry` / `actone.invoke` | **Runtime** spec-driven discovery over the live Extend REST API (`search`/`describe`/`call`/`tags`/`version`), plus SOAP admin + Designer config. Reads open; writes gated (opt-in). See [ActOne Ops](#actone-ops-runtime-discovery). |
...
actone ops soap ...              # curated SOAP admin ops (list/describe/call)
actone ops designer ...          # design-time Designer config (search-types/create/clone/validate/...)
actone ops smoke ...             # pre-ship health check across the MCP surface
```
...
- **Write gate (default-deny):** `call` runs **read** (GET/HEAD) operations freely;
  writes (POST/PUT/DELETE/PATCH, and Designer/SOAP create/update/remove) are refused
  *before* any login unless writes are explicitly enabled — `--allow-write`,
  `ACTONE_ALLOW_WRITES=true`, or an env profile with `allow_writes: true`.
...
`actone_mcp/server.py` (FastMCP, stdio/HTTP) exposes the same surface to MCP clients
(Copilot CLI/VS Code, Claude Code) as **23 tools**: the REST discovery loop
(`search_ops`, `list_ops`, `describe_op`, `invoke_op`, `list_tags`), the curated SOAP
admin tools (`list_soap_operations`, `invoke_soap_operation`), `list_environments`, and
the catalog-driven Designer engine (`designer_*` plus typed `create_*` wrappers).
Writes are gated. Registered in `.vscode/mcp.json` as `actone-ops`; console entry point
`actone-mcp`.
