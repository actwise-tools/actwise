...
**SOAP** catalog (``actone-catalog.json``) that describes the legacy Axis services:
  ...
  * ``objectTypes`` — 275 Designer object types; 100 are exportable and 75 have
                      a resolved create bean in ``createPaths`` (``via`` =
                      ``generic`` addObject or a ``dedicated`` operation).
...
Stdlib only (json); designer-flows.yaml is loaded lazily via pyyaml (project dep).
...
# Documented design-time setup *flows* the catalog cannot express — the "do X before
# Y" order, prerequisites, gotchas (wire formats, pool limits) and doc pointers per
# creatable type. Authored in data/designer-flows.yaml and surfaced through
# describe_type's ``grounding`` block so the agent reads the procedure (and the traps)
# before authoring (see _grounding). ``docQuery`` is fed to the docenter docs MCP;
# ``docUrl`` is a representative page for direct reading.
_FLOWS_CACHE: dict | None = None


def _load_flows() -> dict:
    """Load and cache the per-type Designer flow registry from designer-flows.yaml.

    Keyed by Designer ``typeValue``. Degrades gracefully to an empty registry if the
    file is missing/unreadable (grounding then falls back to generic doc queries)."""
    global _FLOWS_CACHE
    if _FLOWS_CACHE is not None:
        return _FLOWS_CACHE
    from actone.paths import FLOWS
    path = Path(os.environ.get("ACTONE_DESIGNER_FLOWS") or FLOWS)
    flows: dict = {}
    try:
        import yaml  # project dependency (pyyaml)
...
class SoapCatalog:
    ...
    def flow_coverage(self) -> dict:
        """Report how many creatable Designer types have a documented flow
        (data/designer-flows.yaml). Also flags flow keys that are not creatable
        typeValues (typos / stale entries). For `sync-skill` coverage reporting."""
        flows = _load_flows()
        creatable = sorted(t.get("typeValue") for t in self.object_types
                           if self.is_creatable(t.get("typeValue") or ""))
        documented = sorted(tv for tv in creatable if tv in flows)
        missing = sorted(tv for tv in creatable if tv not in flows)
        unknown = sorted(k for k in flows if not self.is_creatable(k))
        return {
            "creatable_total": len(creatable),
            "documented": len(documented),
            "documented_types": documented,
            "missing": missing,
            "unknown_flow_keys": unknown,
        }
    ...
    def describe_type(self, type_value: str) -> dict | None:
        ...
        capability_state = self.capabilities.for_type(type_value)
        out = {
            ...
            "creatable": creatable,
            **self._support_state(creatable, capability_state),
    ...
    def _grounding(self, t: dict) -> dict:
        ...
        seq = _load_flows().get(type_value)
        doc_query = seq.get("docQuery") if seq else None
        out = {
            ...
            "suggestedQueries": [doc_query] if doc_query else ["Setting up %s" % label, label],
        }
        if seq:
            # Documented setup flow (do X before Y) + prerequisites + gotchas for
            # high-value types, authored in data/designer-flows.yaml.
            sequence: dict = {"steps": list(seq.get("steps") or [])}
            for key in ("summary", "prerequisites", "gotchas", "docQuery", "docUrl", "note"):
                if seq.get(key):
    ...
    def _type_brief(self, t: dict) -> dict:
        ...
        capability_state = self.capabilities.for_type(tv)
        return {
            "typeValue": tv,
            "enumName": t.get("enumName"),
            "creatable": creatable,
    ...
    @staticmethod
    def _support_state(creatable: bool, capability_state: dict) -> dict:
        maturity = capability_state["maturity"]
        if maturity == "certified":
            support_state = "certified"
        elif maturity == "verified_read_only":
            support_state = "verified_read_only"
        elif maturity == "experimental":
            support_state = "experimental"
        elif creatable:
            support_state = "catalog_candidate"
        else:
            support_state = "unavailable"
        return {
            "supportState": support_state,
            "catalogCandidate": bool(creatable and maturity == "catalog_only"),
            "capabilityIds": list(capability_state["capabilities"]),
        }
