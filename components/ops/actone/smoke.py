...
def smoke_designer_write(S, env: Optional[str], workflow: str) -> dict:
    ...
    Covers the mechanisms we actually author — the deprecated AlertType adapter
    redirected through the compound work-item recipe, **typed** DDQ, **curated
    SOAP** (BusinessUnit), and the **generic engine** (clone). Every created
    object is removed in a ``finally`` so a partial failure leaves no residue.
    Requires the target env to permit writes. ``workflow`` is retained only for
    call compatibility; the recipe creates an isolated workflow family."""
    bucket = _new_bucket()
    ts = int(time.time())
    at_id = "AW_SMOKE_AT_%d" % ts
    at_field_id = "AW_SMOKE_FIELD_%d" % ts
    at_open_id = "AW_SMOKE_OPEN_%d" % ts
    at_closed_id = "AW_SMOKE_CLOSED_%d" % ts
    at_workflow_id = "AW_SMOKE_WF_%d" % ts
    at_view_id = "AW_SMOKE_VIEW_%d" % ts
    ...
    alert_specification = {
        ...
        "description": "smoke",
        "customFields": [{
            "identifier": at_field_id,
            "label": "Smoke Field",
            "dataType": "string",
            "length": 20,
        }],
        "workflow": {
            "identifier": at_workflow_id,
            "statuses": [
                {
                    "identifier": at_open_id,
                    "name": at_open_id,
                    "state": "Open",
                    "start": True,
                },
                {
                    "identifier": at_closed_id,
                    "name": at_closed_id,
                    "state": "Closed",
                    "issue": "Non_Issue",
                },
            ],
            "transitions": [{"from": at_open_id, "to": at_closed_id}],
        },
        "view": {
            "identifier": at_view_id,
            "fields": ["alertId", "status", at_field_id],
        },
    }

    # -- Legacy adapter: compound recipe -> verify -> compound remove --------
    try:
        res = S.create_alert_type(
            at_id,
            name=at_id,
            description="smoke",
            specification=alert_specification,
            env=env,
        )
        ...
        try:
            cleanup = S.designer_plan(
                "remove_work_item_type", alert_specification, env=env
            )
            if cleanup.get("applicable"):
                S.designer_apply(cleanup, env=env)
        except Exception:  # noqa: BLE001
            pass
