...
Stdlib for transport (urllib); XML parsing goes through defusedxml (XXE- and
entity-expansion-safe) over xml.etree, matching the rest of the package.
"""
...
import xml.etree.ElementTree as ET  # Element type + tree iteration only
...
class SoapClient:
    ...
    # -- generalized (designer) transport -----------------------------------
    @staticmethod
    def _designer_envelope(service: str, operation: str, inner_xml: str) -> str:
        # The Axis *designer* services expect the operation element in the
        # per-service ``urn:<service>`` namespace with encodingStyle on the
        # operation element itself (verified against rcm-designer-mcp's SoapClient),
        # which differs from the curated businessUnitService envelope above.
        return (
            '<?xml version="1.0" encoding="UTF-8"?>'
            '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"'
            ' xmlns:xsd="http://www.w3.org/2001/XMLSchema"'
            ...
            '</urn:%s>'
            '</soapenv:Body></soapenv:Envelope>'
        ) % (service, operation, inner_xml, operation)

    def invoke_raw(self, service: str, operation: str, inner_xml: str) -> str:
        """Low-level RPC/encoded call used by the generalized designer engine
        (designer.py). Reuses the authenticated REST session + 401/403 self-heal
        and returns the raw response XML for the caller to parse. ``inner_xml`` is
        the already-encoded parameter list (self-contained; declares its own
        ``urn:``/``soapenc:`` prefixes on payload elements)."""
        self._c.ensure_login()
        env = self._designer_envelope(service, operation, inner_xml)
        return self._post(service, env)
...
def _parse_response(op_id: str, operation: str, raw: str) -> dict:
    ...
    try:
        # defusedxml: forbids DTD/entity/external refs → blocks XXE + entity-expansion
        # attacks on the (server-controlled but still untrusted) SOAP response body.
