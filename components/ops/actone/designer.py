...
ALERT_DESIGN_SERVICE = "alertDesignService"

# Work-item family object types that must be deleted through their dedicated
# alertDesignService op (the generic removeObject/getRemoveConstraints reject
# them live). Maps type -> (operation, string-identifier parameter name).
# AlertStatusWorkflowDefinition is intentionally absent: it is a generic
# designerRepositoryService object and uses the generic removeObject route.
_WORK_ITEM_REMOVE_OPS = {
    "AlertView": ("removeAlertView", "viewIdentifier"),
    "AlertType": ("removeAlertType", "alertTypeIdentifier"),
    "AlertStatus": ("removeAlertStatus", "alertStatusIdentifier"),
    "AlertCustomizedField": (
        "removeAlertCustomizedField",
...
def _wire_field_name(bean_name: str, field_name: str) -> str:
    """Correct catalog/source member names that differ from the Axis wire bean."""
    if bean_name == "AlertStatus":
        return {
            "statusIdentifier": "identifier",
            "statusName": "name",
        }.get(field_name, field_name)
    if bean_name == "BusinessHierarchyNode" and field_name == "emptyHierArray":
        return "childNodes"
    return field_name
...
def _serialize_fields(catalog: SoapCatalog, bean: dict, args: dict, root: bool) -> str:
    ...
    for f in bean.get("flattenedFields", []) or []:
        ...
        wire_field = _wire_field_name(bean.get("name", ""), field)
        jtype = f.get("type", "")
        ...
        if xsd is not None and not is_array:
            ...
            if value is None or str(value) == "":
                if field == "id" and base in ("int", "long"):
                    parts.append('<%s xsi:type="xsd:%s">-1</%s>'
                                 % (wire_field, base, wire_field))
                if base == "UpdatableEnum":
                    parts.append('<%s xsi:type="urn:UpdatableEnum">Updatable</%s>'
                                 % (wire_field, wire_field))
                continue
            xsi = ("urn:%s" % base) if is_enum else ("xsd:%s" % xsd)
            parts.append('<%s xsi:type="%s">%s</%s>'
                         % (wire_field, xsi, _wire(value, xsd), wire_field))
            continue
        ...
        if nested is not None and value is not None:
            if is_array and isinstance(value, list):
                ...
                parts.append('<%s xsi:type="urn:%s">' % (wire_field, base))
...
def _typed_field_value(values: Mapping, declared_name: str):
    """Read a catalog field by its wire name, accepting the existing friendly
    de-prefixed spelling as a fallback."""
    if declared_name in values:
        return values[declared_name]
    friendly_name = _arg_name(declared_name)
    return values.get(friendly_name)


def _typed_wire_field_name(bean_name: str, declared_name: str) -> str:
    """Return the Axis field name for strict catalog-typed serialization."""
    return _wire_field_name(bean_name, _arg_name(declared_name))


def _validate_typed_fields(bean: dict, values: Mapping, path: str) -> None:
    declared_names = [
        field.get("name", "")
        for field in bean.get("flattenedFields", []) or []
    ]
    accepted_names = set(declared_names)
    accepted_names.update(_arg_name(name) for name in declared_names)
    unknown = sorted(
        (name for name in values if name not in accepted_names),
        key=str,
    )
    if unknown:
        raise DesignerError(
            "unknown field(s) for typed operation input %r (%s): %s"
            % (path, bean.get("name", ""), ", ".join(map(str, unknown))))

    for declared_name in declared_names:
        friendly_name = _arg_name(declared_name)
        if (
            friendly_name != declared_name
            and declared_name in values
            and friendly_name in values
        ):
            raise DesignerError(
                "ambiguous duplicate aliases for typed operation input %r: %s and %s"
                % (path, declared_name, friendly_name))


def _reject_raw_xml(values, path: str) -> None:
    if isinstance(values, Mapping):
        for name, value in values.items():
            if str(name).endswith("_xml"):
                raise DesignerError(
                    "raw XML is not accepted by typed operation input %r"
                    % (path + "." + str(name)))
            _reject_raw_xml(value, path + "." + str(name))
    elif isinstance(values, list):
        for index, value in enumerate(values):
            _reject_raw_xml(value, "%s[%d]" % (path, index))


def _serialize_typed_fields(catalog: SoapCatalog, bean: dict,
                            values: Mapping, path: str) -> str:
    _validate_typed_fields(bean, values, path)
    parts: list[str] = []
    for field in bean.get("flattenedFields", []) or []:
        declared_name = field.get("name", "")
        wire_name = _typed_wire_field_name(bean.get("name", ""), declared_name)
        value = _typed_field_value(values, declared_name)
        if value is None:
            continue
        parts.append(_serialize_typed_value(
            catalog, wire_name, field.get("type", ""), value,
            path + "." + declared_name,
        ))
    return "".join(parts)


def _serialize_typed_value(catalog: SoapCatalog, name: str, java_type: str,
                           value, path: str) -> str:
    """Serialize one catalog-declared scalar, bean, or array without raw XML."""
    is_array = java_type.endswith("[]")
    base = _base_type(java_type)
    xsd = _xsd_type(base)

    if is_array:
        if not isinstance(value, list):
            raise DesignerError("typed operation input %r must be an array" % path)
        item_type = ("xsd:%s" % xsd) if xsd is not None else ("urn:%s" % base)
        parts = [
            '<%s soapenc:arrayType="%s[%d]" xsi:type="soapenc:Array">'
            % (name, item_type, len(value))
        ]
        if xsd is not None:
            xsi = ("urn:%s" % base) if base.endswith("Enum") else item_type
            parts.extend(
                '<item xsi:type="%s">%s</item>' % (xsi, _wire(item, xsd))
                for item in value
            )
        else:
            nested = catalog.find_bean(base)
            if nested is None:
                raise DesignerError(
                    "catalog type %r for typed operation input %r is unsupported"
                    % (java_type, path))
            for index, item in enumerate(value):
                if not isinstance(item, Mapping):
                    raise DesignerError(
                        "each element of typed operation input %r must be an object (%s)"
                        % (path, base))
                parts.append('<item xsi:type="urn:%s">' % base)
                parts.append(_serialize_typed_fields(
                    catalog, nested, item, "%s[%d]" % (path, index)))
                parts.append("</item>")
        parts.append("</%s>" % name)
        return "".join(parts)

    if xsd is not None:
        xsi = ("urn:%s" % base) if base.endswith("Enum") else ("xsd:%s" % xsd)
        return '<%s xsi:type="%s">%s</%s>' % (
            name, xsi, _wire(value, xsd), name)
    ...
    if not isinstance(value, Mapping):
        raise DesignerError(
            "typed operation input %r must be an object (%s)" % (path, base))
    return (
        '<%s xsi:type="urn:%s">%s</%s>'
        % (name, base, _serialize_typed_fields(catalog, nested, value, path), name)
    )
...
def _to_obj(el: ET.Element, pool: dict, seen: frozenset = frozenset()):
    ...
    is_axis_array = (
        (xtype is not None and _local(xtype) == "Array")
        or any(_local(name) == "arrayType" for name in el.attrib)
    )
    if is_axis_array:
        return [_to_obj(child, pool, seen) for child in children]
    if not children:
...
def _parse(raw: str, operation: str,
           out_param: str | Sequence[str] | None) -> dict:
    """Parse a designer SOAP response. Returns dict with ok/status/messages and,
    when ``out_param`` is given, ``out`` = the first resolved out parameter.
    A sequence additionally returns every resolved value under ``outputs``."""
    ...
    out_params = ([out_param] if isinstance(out_param, str)
                  else list(out_param or []))
    if out_params:
        outputs = {name: None for name in out_params}
        for c in resp:
            name = _local(c.tag)
            if name in out_params:
                outputs[name] = _to_obj(c, pool)
        if len(out_params) == 1 and outputs[out_params[0]] is None:
            candidates = [
                c for c in resp
                if not _local(c.tag).endswith("Return")
            ]
            if len(candidates) == 1:
                outputs[out_params[0]] = _to_obj(candidates[0], pool)
        result["out"] = outputs.get(out_params[0])
        if not isinstance(out_param, str):
...
class DesignerClient:
    ...
    def create_object(self, type_value: str, fields: dict) -> dict:
        ...
        if path and path.get("via") == "dedicated":
            return self._create_via_op(type_value, path, fields)
    ...
    def update_typed_object(self, type_value: str, identifier: str, fields: dict) -> dict:
        """Update an existing object through the same create-route family (dedicated
        vs generic) its type actually uses to create objects, so update stays
        consistent with :meth:`create_object` as the catalog evolves.

        A type whose real create route is dedicated (e.g. AlertView routes through
        ``alertDesignService.addAlertView``, not the generic ``addObject``) is
        updated through the paired dedicated op discovered from catalog operation
        metadata (``addX`` -> ``updateX`` on the same service), not a hardcoded
        per-type mapping. A type created through the generic route is updated
        through the generic ``updateObject`` op instead."""
        t = self.catalog.find_type(type_value)
        path = self.catalog.pick_create_path(t) if t else None
        dedicated_path = None
        if path and path.get("via") == "dedicated":
            dedicated_path = path
        elif path and path.get("via") == "generic":
            bean = self.catalog.find_bean(path.get("bean"))
            if bean and not _has_object_info(bean):
                dedicated_path = _dedicated_create_path(t)
        if dedicated_path:
            return self._update_via_op(type_value, dedicated_path, identifier, fields)
        xml = build_object_xml(self.catalog, type_value, fields)
        return self.update_object(type_value, identifier, xml)

    def _update_via_op(self, type_value: str, create_path: dict, identifier: str,
                       fields: dict) -> dict:
        """Update through the dedicated op paired with a dedicated create op (e.g.
        ``addAlertView`` -> ``updateAlertView``), derived from catalog operation
        metadata instead of a hardcoded per-type mapping."""
        service, create_operation = create_path["op"].split(".", 1)
        if not create_operation.startswith("add"):
            raise DesignerError(
                "cannot derive an update operation from create op %r"
                % create_operation)
        update_operation = "update" + create_operation[len("add"):]
        op = self.catalog.find_operation(service, update_operation)
        if not op:
            raise DesignerError(
                "%r has no %s.%s update operation" % (type_value, service, update_operation))
        in_params = [p for p in op.get("parameters", []) or [] if p.get("mode") == "in"]
        id_param = next((p for p in in_params if p.get("type") == "String"), None)
        bean_param = next((p for p in in_params if p is not id_param), None)
        if not id_param or not bean_param:
            raise DesignerError(
                "%s.%s has an unsupported parameter shape for update"
                % (service, update_operation))
        identifier_xml = '<%s xsi:type="xsd:string">%s</%s>' % (
            id_param["name"], _esc(identifier), id_param["name"])
        bean_xml = build_object_xml(
            self.catalog, type_value, fields,
            param_name=bean_param["name"], path=create_path,
        )
        return self._call(service, update_operation, identifier_xml + bean_xml, None)
    ...
    def remove_work_item_object(self, type_value: str, identifier: str) -> dict:
        """Delete a work-item family object through its authoritative service op.

        The generic ``designerRepositoryService.removeObject`` (and its
        ``getRemoveConstraints`` companion) reject the alert-design object types
        live with "<Type> is not supported for this operation"; those objects are
        owned by ``alertDesignService`` and must be removed through its dedicated
        ``removeAlert*`` ops. Only ``AlertStatusWorkflowDefinition`` is a generic
        repository object and keeps the generic route. Each op fails closed
        (ACMResult ``status=false`` -> :class:`DesignerError`) when the object is
        still referenced by anything outside the family, so this is the
        server-enforced safety point for removal."""
        route = _WORK_ITEM_REMOVE_OPS.get(type_value)
        if route is None:
            return self.remove_object(type_value, identifier)
        operation, param = route
        inner = '<%s xsi:type="xsd:string">%s</%s>' % (
            param, _esc(identifier), param,
        )
        return self._call(ALERT_DESIGN_SERVICE, operation, inner, None)
    ...
    def call_typed_operation(self, service: str, operation: str,
                             inputs: Mapping[str, object]) -> dict:
        """Invoke a catalog operation from typed Python input values.

        Every declared ``mode=in`` parameter is required and serialized in
        catalog order. Complex values are mappings, arrays are lists, and raw
        XML fallbacks are deliberately unavailable; use :meth:`call_operation`
        when an operation requires hand-authored XML."""
        op = self.catalog.find_operation(service, operation)
        if not op:
            raise DesignerError("unknown SOAP operation %s.%s" % (service, operation))
        if not isinstance(inputs, Mapping):
            raise DesignerError("typed operation inputs must be a mapping")

        in_params = [
            param for param in op.get("parameters", []) or []
            if param.get("mode") == "in"
        ]
        expected = [param.get("name") for param in in_params]
        unknown = sorted(
            (name for name in inputs if name not in expected),
            key=str,
        )
        if unknown:
            raise DesignerError(
                "unknown input parameter(s) for %s.%s: %s"
                % (service, operation, ", ".join(map(str, unknown))))
        missing = [name for name in expected if name not in inputs]
        if missing:
            raise DesignerError(
                "missing required input parameter(s) for %s.%s: %s"
                % (service, operation, ", ".join(missing)))

        _reject_raw_xml(inputs, "%s.%s" % (service, operation))
        inner_xml = "".join(
            _serialize_typed_value(
                self.catalog, param["name"], param.get("type", ""),
                inputs[param["name"]], param["name"],
            )
            for param in in_params
        )
        out_params = tuple(
            param["name"] for param in op.get("parameters", []) or []
            if param.get("mode") == "out"
        )
        return self._call(service, operation, inner_xml, out_params)
