...
| `list_ops` | Enumerate the operation surface, **paged** (default 50/page; `offset`/`limit`, `tag`, `reads_only`). |
...
| `designer_search_capabilities` / `designer_inspect` | Discover deep authoring recipes, maturity, and build-specific certification evidence. |
...
| `designer_diff_exports` / `designer_compare_environments` | Diff exported JSON without login, or independently export and compare two verified environments (READ). |
| `designer_plan` / `designer_apply` / `designer_verify` | Shared lifecycle for Platform Lists, work-item families, DDQs, modern work-item presentation, and business-unit hierarchies; writes require both the environment gate and a live-certified build. |
...
| `create_drill_down_query` | Typed DDQ convenience wrapper (gated). |
| `create_alert_type` | **Deprecated compatibility adapter**. Requires a complete `create_work_item_type` specification and delegates to its certified plan/apply/verify path; no direct AlertType SOAP create remains. |
| `create_case_type` | Legacy typed case-type create (gated; modern config uses case items). |
...
> `manage_platform_list` follows that split explicitly: Designer SOAP creates/reuses
> the `InternalList` schema, while `getFilteredListData`, `addItemsToList`, and
> fail-closed `updateItems` handle item reads/mutations. It is certified on 10.2.0.19
> with no `verifiedBuilds`, so
> apply remains certification-blocked until a build completes live write, read-back,
> desired-item verification, and zero-mutation re-plan evidence.
>
> Three additional capabilities have 10.2.0.20 offline-contract coverage.
> `manage_drill_down_query` and `manage_business_unit_hierarchy` are certified on
> 10.2.0.19; `configure_work_item_presentation` remains experimental capability-wide
> (only its field-policy route is live-certified; the relations and alert-type-field
> routes are documented build-blocked on 10.2.0.19).
> `manage_drill_down_query` requires trusted connection evidence (and positive
> zero-reference evidence for updates), `configure_work_item_presentation` is
> update-only and requires renderer inventory for formatter references, and
> `manage_business_unit_hierarchy` supports only create/exact reuse with exclusively
> new business units. Exact reuse additionally requires
> `getBusinessUnitAllHierarchies` to prove each hydrated BU belongs only to the
> current hierarchy. Hierarchy route metadata is catalog-derived; updates, removals,
> shared BUs, roles, legacy `CaseType`, invented `CaseItemType`, JDBC connection
> authoring, and DART report-definition authoring are not supported. See the official
...
### Deploying the hosted MCP (Copilot Studio)

Copilot Studio talks to a **remote** Streamable-HTTP MCP (not the local stdio one) via a
Power Platform custom connector (`connector-swagger.json` → `host: actwise-ops.nousy.ai`,
`X-API-Key`). All 23 tools are discovered dynamically over the MCP protocol. Ship changes
by rebuilding and redeploying the container:

```powershell
# 1. Build from the repo root (the Dockerfile copies core + actone + actone_mcp)
docker build -f components/ops/actone_mcp/Dockerfile -t <registry>/actwise-actone-ops:latest .
docker push <registry>/actwise-actone-ops:latest

# 2. Redeploy the actwise-ops.nousy.ai service to the new image, keeping the env vars:
#    ACTONE_URL / ACTONE_USER / ACTONE_PASSWORD / ACTONE_CONTEXT_ROOT + ACTONE_PROXY_API_KEY
```

**Gateway (Cloudflare / reverse proxy) — required for Streamable HTTP (SSE):** disable
response buffering on `/mcp`, raise the read timeout (~300s), and keep ≥1 warm instance
(no scale-to-zero). Without this, large streamed tool results return HTTP 502/520 with an
empty stream (`frames=0`); `list_ops` is paginated (50/page) to keep responses small.

**Verify after redeploy:** `curl https://actwise-ops.nousy.ai/healthz` → `{"status":"ok",…}`,
then in Copilot Studio confirm all **23 tools** appear and `list_soap_operations`,
the `create_alert_type` migration response, and a `list_ops` page all succeed. A **stale image** shows only the 8
REST/SOAP tools (no `designer_*`) and SOAP fails with a missing `defusedxml` module — the
signal to rebuild. See the [MCP tunnel runbook](../../docs/runbooks/2026-07-10-actwise-mcp-tunnel-runbook.md).
