...
def download(
    ...
    for f in files:
        ...
        if not no_verify and f.md5:
            # Download-integrity check against the vendor-supplied MD5, not a
            # security control — mark it so as not to trip FIPS/security scanners.
            got = hashlib.md5(path.read_bytes(), usedforsecurity=False).hexdigest()
