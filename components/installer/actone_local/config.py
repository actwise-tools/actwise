...
class Config:
    ...
    # Development mode (acm.ini `actimize.mode`). When True, ActOne returns
    # stack traces / DB errors to the caller instead of the generic
    # "contact your System Administrator" message, and reloads Velocity/XSL
    # templates per call. Ideal for a local dev/demo box; set False to mimic
    # production behaviour. Requires an ActOne restart to take effect.
    dev_mode: bool = True

    # Docker restart policy applied to BOTH the DB and the ActOne container so
    # they come back on machine/Docker Desktop boot (like the tunnels/portal).
    # "unless-stopped" survives reboots but respects an explicit `down`. Set to
    # "no" to disable auto-start. Ordering (DB before app) is handled by the DB
    # healthcheck + the app's restart-on-failure retry, not by Docker itself.
    ...
    # Pre-extracted RCM Encryption Tool (Utilities/{etc,lib}). Used by
    # `encrypt-config` as a fallback when the multi-GB `package_zip` has been
    # deleted to reclaim disk — the ~73 MB Utilities/lib is enough to encrypt.
    utilities_dir: str = "packages/ActOne-10.2.0-Utilities/Utilities"
    ...
    @property
    def logs_dir(self) -> Path: return self.work_dir_path / "logs"
    ...
    @property
    def utilities_dir_path(self) -> Path: return self.p(self.utilities_dir)

    @property
    def utilities_lib_path(self) -> Path: return self.utilities_dir_path / "lib"
