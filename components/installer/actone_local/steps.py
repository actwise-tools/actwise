...
def _apply_restart_policy(name: str, policy: str) -> None:
    """Bring an already-created container under a restart policy without recreating it.

    Restart policy is normally baked at `docker run` time; `docker update` lets us
    apply it to pre-existing containers so they auto-start on boot too.
    """
    if not policy or policy == "no":
        return
    sh(["docker", "update", "--restart", policy, name])
...
def doctor(cfg: Config) -> list[Step]:
    ...
    enc_pkg = cfg.package_zip_path.exists()
    enc_lib = cfg.utilities_lib_path.exists() and any(cfg.utilities_lib_path.glob("*.jar"))
    steps.append(Step(enc_pkg or enc_lib, "encryptor",
                      "from package" if enc_pkg else
                      (f"from {cfg.utilities_dir}/lib" if enc_lib else
                       "MISSING — need package_zip or a pre-extracted Utilities/lib for `encrypt-config`")))
...
def db_up(cfg: Config) -> Step:
    ...
    if state == "running":
        _apply_restart_policy(cfg.db.container, cfg.restart_policy)
    ...
    r = sh([
        ...
        "--health-cmd", f"pg_isready -U {cfg.db.user} -d {cfg.db.name}",
        "--health-interval", "10s", "--health-timeout", "5s",
        "--health-retries", "5", "--health-start-period", "10s",
...
def _acm_ini_text(cfg: Config, password: str, iv: str | None = None) -> str:
    ...
    # actimize.mode: development => stack traces / DB errors surface to the caller
    # (instead of the generic "contact your System Administrator" 500) and
    # templates reload per call. Default production. Requires restart to change.
    lines.append(f"actimize.mode = {'development' if cfg.dev_mode else 'production'}")
    return "\n".join(lines) + "\n"
...
def encrypt_config(cfg: Config) -> Step:
    ...
    Prefers the encryptor bundled in ``package_zip``; falls back to a
    pre-extracted ``utilities_dir`` (``Utilities/{etc,lib}``) so this still works
    after the multi-GB package has been deleted to reclaim disk.
    """
    ok, msg = docker_ok()
    if not ok:
        return Step(False, "encrypt-config", msg)

    if cfg.package_zip_path.exists():
        try:
            _extract_encryptor(cfg)
        except Exception as e:
            return Step(False, "encrypt-config", f"failed to extract encryptor: {e}")
        enc_dir = cfg.encryptor_dir
    elif cfg.utilities_lib_path.exists() and any(cfg.utilities_lib_path.glob("*.jar")):
        enc_dir = cfg.utilities_dir_path
    else:
        return Step(False, "encrypt-config",
                    f"no encryptor available — package {cfg.package_zip} is missing and "
                    f"no pre-extracted Utilities/lib at {cfg.utilities_dir}. Restore either "
                    f"(the ~73 MB Utilities/lib is enough).")

    enc = str(enc_dir.resolve())
    java = (
        "cd /enc && java "
...
def run_container(cfg: Config) -> Step:
    ...
    if _container_state(cfg.container) == "running":
        _apply_restart_policy(cfg.container, cfg.restart_policy)
    ...
    # The image's /usr/local/tomcat/bin/acm is root-owned (755) while the
    # container runs as uid 1000 (tomcat), so ActOne can't create its
    # actimizeLogs dir — that breaks EVERY file appender (no logs at all, and
    # runtime errors get masked behind the generic 500). Bind-mounting a
    # host-writable dir there fixes logging and surfaces the logs on the host.
    ...
    cmd = [
        ...
        "-v", f"{cfg.logs_dir}:/usr/local/tomcat/bin/acm/actimizeLogs",
