...
Endpoints:
  ...
  POST /ingest              Door 3: accept a client-side-captured _SESSION, authorized by the state
...
from docenter.cli import zoomin_login_complete
...
def connect(state: str, request: Request):
    ...
    return HTMLResponse(_connect_page(state, ls.user_id, already, _public_base(request)))
...
def ingest(body: IngestRequest):
    """Door 3 (client-side capture): accept a `_SESSION` captured on the user's own
    managed device and store it, authorized by the one-time signed ``state``.

    Employee-SSO topology: SSO/Conditional-Access only passes on a managed device,
    so the browser capture runs client-side (``docenter auth connect``) and this
    endpoint just receives the result — no hosted browser, so it can live on AWS.
    The user_id comes from the SIGNED state (R2 — never a client field), and the
    payload must be a COMPLETED login (``zoomin_login_complete``) so a pre-auth or
    forged cookie set cannot be stored. One-time: the nonce is consumed on success."""
    secret = broker_secret()
    if not secret:
        raise HTTPException(status_code=503, detail="broker not configured")
    try:
    ...
    cookies = (body.payload or {}).get("data", {}).get("cookies")
    if not isinstance(cookies, list) or not cookies:
        raise HTTPException(status_code=400, detail="payload missing data.cookies[]")
    found = zoomin_login_complete(cookies)
    if not found:
        raise HTTPException(
            status_code=400,
            detail="payload is not a completed login (need ZD__userAuthenticated=true + _SESSION)",
        )
    ...
    save_user_cookie_data(ls.user_id, body.payload)
    return {"status": "connected", "user_id": ls.user_id,
            "expires": int(found.get("expires", -1) or -1)}
...
def _connect_page(state: str, user_id: str, already_used: bool, public_base: str = "") -> str:
    safe_user = html.escape(user_id)
    safe_state = html.escape(state)
    safe_base = html.escape(public_base.rstrip("/") or "https://<broker-url>")
    if already_used:
...
<div class="door">
  <h3>NICE employee — connect from your own device</h3>
  <p class="muted">If SSO in the hosted browser is blocked by device policy, capture your
  session on your own managed machine instead. Install the tool
  (<code>pip install actwise</code>), then run:</p>
  <pre style="background:#f5f5f7;padding:.7rem;border-radius:6px;overflow:auto"><code>docenter auth connect --broker {safe_base} --state {safe_state}</code></pre>
  <p class="muted">A browser opens for SSO on your device; the CLI sends the captured
  session here. Return to the chat when it prints “Connected”.</p>
</div>
