...
```
Skills are instructions only — they drive the ActWise CLIs, so install the `actwise` distribution too. This skill requires the **`actwise-data`** console script and the **`actwise-data-mcp`** server (legacy `actone-data` / `actone-data-mcp` names still work). Configure the DB via `--profile`, a libpq `--dsn`/`ACTWISE_DATA_DSN`, or `ACTONE_DB_*` env vars.
