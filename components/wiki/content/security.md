...
Config is resolved by precedence — the first location that supplies a file wins — while
credentials live only in gitignored secrets files:

```mermaid
flowchart LR
    E["$ACTWISE_CONFIG_DIR"]
    W["cwd"]
    H["~/.actwise"]
    R["dev repo root"]
    S[("Secrets<br/><small>.env · *.secrets.yaml — gitignored</small>")]
    E -->|first match wins| W --> H --> R
    H -.->|passwords| S
```
