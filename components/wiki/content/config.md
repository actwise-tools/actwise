...
| Data (read-only SQL) | `actwise-data.yaml` | `actwise-data.secrets.yaml` |
...
- **Data** — `actwise-data.yaml` lists one DB profile per environment (host, port,
  name, read-only user, schema). *(The legacy `actone-data.yaml` name is still
