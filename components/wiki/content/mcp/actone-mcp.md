...
> Turns a live ActOne instance into a discoverable REST **and** design-time
> Designer (SOAP) surface — a `search → describe → invoke` loop with writes gated
> by default.
...
write is refused unless the operator opted in server-side. It also exposes the
legacy **SOAP** surface the REST API lacks — a curated admin slice (e.g. create
Business Unit) plus a catalog-driven **Designer** engine that can create, clone,
read, update, remove, and validate any of ActOne Designer's ~275 configuration
object types (alert/case types, workflows, views, and Drill Down Queries).
...
Enumerated from `components/ops/actone_mcp/server.py` — **32** `@mcp.tool`
registrations (FastMCP). The **runtime REST + curated SOAP** tools:

| Tool | What it does |
|------|--------------|
| `search_ops` | Discover REST operations by keyword (operationId / summary / tags / path); `reads_only` to filter to GETs. |
| `list_ops` | Enumerate the operation surface, **paged** (default 50/page); filter by `tag` / `reads_only`, page with `offset`/`limit`, or `group` all by tag. |
...
The **design-time Designer (SOAP)** tools — a generic, catalog-driven engine over
ActOne Designer's object types (one engine, not a static tool per type):

| Tool | What it does |
|------|--------------|
| `designer_search_types` | Discover Designer object types by keyword; `creatable_only` to filter to creatable ones. |
| `designer_describe_type` | Show a type's create path + payload-bean field schema, `references`/`composes` relations, and a **`grounding`** block pointing at the docs MCP. |
| `designer_search_operations` | Search the raw SOAP operations across the 22 Axis services (offline). |
| `designer_search_capabilities` / `designer_inspect` | Discover deep recipes, maturity, and target-build evidence. |
| `designer_export_state` / `designer_compare_state` | Export normalized `create_work_item_type` state from a verified build or compare it with a desired specification (READ). |
| `designer_diff_exports` / `designer_compare_environments` | Diff intact exported JSON documents without login, or independently collect and compare two verified environments (READ). |
| `designer_plan` / `designer_apply` / `designer_verify` | Shared deep capability lifecycle for Platform Lists, work-item families, DDQs, modern work-item presentation, and business-unit hierarchies; apply is blocked until a build is live-certified. |
| `designer_list_objects` / `designer_get_object` | List existing objects of a type / fetch one by identifier (READ). |
| `designer_get_remove_constraints` | List delete constraints for an object (READ). |
| `designer_create_object` | Create any creatable type from a field dict (WRITE, gated). |
| `create_drill_down_query` | Typed convenience wrapper to create a **Drill Down Query** (WRITE, gated). |
| `create_alert_type` | **Deprecated compatibility adapter** requiring a complete `create_work_item_type` specification; delegates to its guarded plan/apply/verify path (WRITE, gated). |
| `create_case_type` | Typed **case type** create (WRITE, gated) — **legacy** (modern config uses case items / work-item types). |
| `designer_clone_object` | Copy an object under a new identifier with field overrides (WRITE, gated). |
| `designer_update_object` / `designer_remove_object` | Save a modified payload / delete an object (WRITE, gated). |
| `designer_validate_object` | Server-side validate a payload without saving (READ). |
| `designer_call_operation` | Escape hatch: invoke any of the 335 catalog operations (write-gated — unclassified). |

> **REST-first convention.** The Designer/SOAP tools cover the **design-time**
> config surface the Extend REST API does not. When a REST equivalent exists
> (`search_ops`/`describe_op`/`invoke_op`), **prefer REST**; use `designer_*` only
> for design-time objects with no REST operation.
>
> **Grounding convention.** The catalog gives a type's field *shape*, not the
> product *procedure*. `designer_describe_type` returns a `grounding` block; before
> authoring, follow its `suggestedQueries` into the ActWise docs MCP (docenter
> `search_docs`/`get_page`) to learn the setup **sequence**, prerequisites, and
> server-enforced mandatory fields the catalog can't express (e.g. a DrillDownQuery
> needs an existing JDBC connection), then create any `references` objects first.
>
> `create_work_item_type` and `remove_work_item_type` are live-certified on 10.2.0.19
> (removal deletes alert-design objects via dedicated `alertDesignService` remove ops
> and the workflow definition via generic `removeObject`, in reverse dependency order,
> fail-closed). `manage_drill_down_query` and `manage_business_unit_hierarchy` are
> certified on
> 10.2.0.19; `configure_work_item_presentation` remains experimental, with only
> its field-policy route live-certified (its relations and alert-type-field routes are
> documented build-blocked on 10.2.0.19). All three retain 10.2.0.20 offline-contract
> coverage. DDQ planning requires trusted connection
> evidence; updates also require positive zero-reference evidence. Presentation is
> update-only and formatter references require renderer inventory; legacy `CaseType`
> is rejected and `CaseItemType` is never invented. Hierarchy supports create/exact
> reuse only, maps the catalog's `emptyHierArray` member to the live Axis
> `childNodes` wire field, and requires
> `getBusinessUnitAllHierarchies` sole-membership evidence for every reused BU.
> Shared BUs, unknown memberships, updates, removals, and roles are blocked. These
> capabilities do not claim JDBC connection or
> DART report-definition authoring. Official references:
> [DDQ connection](https://docs.niceactimize.com/bundle/Actimize_ActOne_10.2_Implementer_Guide/page/Content/Platform/RCM/RCM_DART/DART_Implementation/Defining_the_Database_Connection.htm)
...
Enumerate the operation surface, **paged** (default 50/page) — unlike `search_ops`.
Filter by `tag` and/or `reads_only`, walk pages with `offset`/`limit`, or set
`group=True` to organize all ops by tag. Pagination keeps a single streamed
response small enough for remote/gateway MCP transports (Copilot Studio).

```mermaid
flowchart TD
    A["list_ops(reads_only?, tag?, group?, offset?, limit?)"] --> REG["_reg() (offline)"]
    REG --> G{group?}
    G -->|yes| GR["reg.grouped(reads_only)<br/>→ groups {tag: [ops]}"]
    G -->|no| FL["reg.list_ops(reads_only, tag)[offset:offset+limit]<br/>→ page + next_offset"]
