...
The server exposes **17 read-only tools** across two surfaces:
...
Enumerated from `components/data/actone_data_mcp/server.py` — **17 `@mcp.tool`
...
| `get_rules` | The solution's **rule pack**: query conventions, preferred/deprecated views, masked (PII) columns, and any solution-specific constraints the model must honor. |
...
| `perf_report` | Scored recommendation report rolling up all checks — each finding carries severity, evidence, fix, and a `source_url` doc citation. Accepts `solution` to scope table/index/vacuum checks to that solution's schema. |
...
| WLF / IFM | `wlf_lm` / `ifm_app` | draft | Package-DDL-derived; pending live validation. |
