...
only. Reads `actwise-data.yaml` via `config.list_profiles()`; passwords are never
returned.

```mermaid
flowchart TD
    A["list_environments()"] --> C["config.list_profiles()<br/>actwise-data.yaml"]
...
| IFM IDB (data) | `ff_idb_data` | ✅ | Investigation Database — where ingested transactions & analytics results land (DART/reporting). 155 tables + 206 `v_ff_*`/`v_acm_*` views (view columns live-typed on `fhba`; view + column descriptions doc-mined from the IFM 11.2.1 Implementer Guide — 266/361 objects described). Separate DB. |
...
(`actwise-data-mcp = "actone_data_mcp.server:main"`; the legacy
`actone-data-mcp` name is still installed as an alias):
...
The HTTP transport is **self-hosted**; when `ACTWISE_DATA_PROXY_API_KEY` is set it
enforces an `X-API-Key` header (mandatory for any tunnelled deployment — how the
ActWise Data agent reaches it). The legacy `ACTONE_DATA_PROXY_API_KEY` env var is
still honored as a fallback.

## Self-host setup

1. **Configure a database profile.** Copy
   `components/data/actone-data.example.yaml` to `~/.actwise/actwise-data.yaml`
   (resolution order: `$ACTWISE_CONFIG_DIR` → cwd → `~/.actwise` → repo root; the
   legacy `actone-data.yaml` name is still resolved as a fallback) and
...
2. **Provide the password** out-of-band: `ACTONE_DB_PASSWORD__<PROFILE>` (profile
   name upper-cased, non-alphanumerics → `_`) or an `actwise-data.secrets.yaml`
   beside the config.
...
5. **Shared HTTP deploy:** set `ACTWISE_DATA_PROXY_API_KEY` and require the
