...
```mermaid
flowchart TB
    core["core<br/><small>actwise.paths — shared config</small>"]
    subgraph caps["Capability buckets"]
        direction LR
        docenter["docenter<br/><small>docs portal</small>"]
        ops["ops<br/><small>live ActOne REST</small>"]
        data["data<br/><small>read-only SQL</small>"]
        utils["utils<br/><small>Java utilities</small>"]
        nicedl["nicedl<br/><small>download media</small>"]
        installer["installer<br/><small>install + local ActOne</small>"]
    end
    core -.->|config resolution| docenter & ops & data & utils & nicedl & installer
```
