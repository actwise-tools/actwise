...
- **Auth.** Portal access uses a Zoomin `_SESSION` cookie in
  `browser-profile/session-cookies.json`, minted by `docenter auth login` — or,
  when hosted, self-minted by the MCP on first use (cold-start bootstrap) from
  the account email/password, so no cookie needs to be seeded by hand.
