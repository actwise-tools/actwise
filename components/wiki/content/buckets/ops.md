...
| `actone_mcp` | The `actone-mcp` MCP server (FastMCP) — surfaces the runtime discovery loop, curated SOAP admin ops, and the catalog-driven Designer engine as tools. |
...
- **MCP:** [`actone-mcp`](../mcp/actone-mcp.md) — discover / describe / invoke
  live REST and curated SOAP operations, plus a catalog-driven Designer engine,
  with writes gated.
...
- **Designer, design-time.** A catalog-driven Designer engine (from a vendored
  `actone-catalog.json`) can create / clone / read / update / remove / validate any
  of ~275 Designer object types (alert & case types, workflows, views, Drill Down
  Queries) generically. **REST-first:** prefer a REST op when one exists; use
  Designer only for design-time objects REST doesn't cover. `designer_describe_type`
  returns a **grounding** block that points the agent at the docs MCP for the setup
  sequence and prerequisites the catalog can't express.
