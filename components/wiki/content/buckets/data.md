...
- **CLI:** [`actone-data`](../cli/actone-data.md) — `ping`, `version`, `schema`,
  `solutions`, `query validate` / `query run`, `rules`, `perf`, `audit`, `env`,
  `docs`, `eval`.
- **MCP:** [`actone-data-mcp`](../mcp/actone-data-mcp.md) — **20 read-only tools**
  across two surfaces: NL-to-SQL grounding + execution (list solutions, schema
  summary, rules, list/describe, validate, run, list/detect business solutions —
  all solution-aware) and performance diagnostics (health, indexes, vacuum,
  config, scored report, explain).
...
- **Rule packs + PII masking.** Per-solution rule packs carry query conventions
  ...
  `***REDACTED***` at result time and reported in `run_query`. The rules are
  **doc-mined from the Actimize Implementer/Installation guides** — a base ActOne
  pack (inherited by every solution) encodes universal best practices (prefer
  documented views over base tables, prefer the `2`-suffixed join-id views over
  deprecated identifier views, `*_join_id` is join-only, filter `fl_deleted=0` /
  `fl_archive=0`, avoid `SELECT *`), and each solution layers its own view
  families, status flags, and masked columns on top.
