...
`actone-utils` · `actone-utils-mcp` · `actwise-data` · `actwise-data-mcp` · `ndc` ·
`actimize-installer` · `actone-local`. *(The data component is also installed under
its legacy `actone-data` / `actone-data-mcp` names.)*
...
copy components\data\actone-data.example.yaml         $HOME\.actwise\actwise-data.yaml
copy components\data\actone-data.secrets.example.yaml $HOME\.actwise\actwise-data.secrets.yaml
```
...
- **`actwise-data.yaml`** — a DB profile per environment (host, port, name, read-only
  user, schema). Passwords go in `actwise-data.secrets.yaml`. *(The legacy
  `actone-data.yaml` / `actone-data.secrets.yaml` names are still resolved as a fallback.)*
...
```json
{
  "mcpServers": {
    ...
    "actone-data":   { "command": "actwise-data-mcp" }
  }
}
```
