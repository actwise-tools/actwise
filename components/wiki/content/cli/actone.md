...
| `ops` | Spec-driven runtime ops over the ActOne Extend REST API (discovery: `search`/`describe`/`call`). Reads open; writes gated (opt-in). |
...
| `ops designer` | Catalog-driven ActOne **Designer** ops — create/read/update/clone/remove/validate any of ~275 config object types. |
...
`ops designer` is a catalog-driven sub-group (design-time config surface). **Prefer
REST** when an equivalent exists; use these only for design-time objects REST can't
reach:

| Sub-command | Description |
| --- | --- |
| `ops designer search-types` | Find Designer object types by keyword (offline). |
| `ops designer describe-type` | Show a type's create path, field schema, relations, and a **grounding** hint pointing at the docs MCP. |
| `ops designer search-ops` | Find raw SOAP operations across the 22 Axis services (offline). |
| `ops designer list` / `get` / `constraints` | Read objects of a type / one object / delete constraints. |
| `ops designer create` | Create any creatable type from `--fields` JSON (WRITE, `--allow-write`). |
| `ops designer create-ddq` | Convenience: create a Drill Down Query (WRITE, `--allow-write`). |
| `ops designer create-alert-type` | Convenience: create an alert type (WRITE, `--allow-write`). |
| `ops designer create-case-type` | Convenience: create a case type (WRITE, `--allow-write`) — **legacy**. |
| `ops designer clone` | Copy an object under a new identifier with `--overrides` (WRITE, `--allow-write`). |
| `ops designer update` | Save a modified object payload XML (WRITE, `--allow-write`). |
| `ops designer validate` | Server-side validate a payload without saving (READ). |
| `ops designer remove` | Delete an object by type + identifier (WRITE, `--allow-write`). |
| `ops designer call` | Escape hatch: invoke any catalog SOAP op (reads free; writes `--allow-write`). |
