...
Installed with the `actwise` distribution. Also available under the `actwise-data`
alias (the internal `actone-data` name stays working). Configure a DB profile in
`actwise-data.yaml` (host, port, name, **read-only** user, schema) with the password in
`actwise-data.secrets.yaml` — the legacy `actone-data.yaml` / `actone-data.secrets.yaml`
names are still resolved as a fallback; see [Install](../install.md).
