...
```mermaid
flowchart TB
    subgraph orch["Orchestrator — Actwise-Main-Dev"]
        direction TB
        O["Router agent"]
        OD["Docs child"]
        OA["Data child"]
        OP["Ops child"]
        O -->|delegates| OD & OA & OP
    end
    subgraph single["Single agent — ActWise-Main1-Dev"]
        direction TB
        S["One agent<br/><small>self-routing</small>"]
        M1["docenter MCP"]
        M2["actone-data MCP"]
        M3["actone MCP"]
        S --> M1 & M2 & M3
...
*A/B tested: the single-agent variant scored **92%** in **14:02**, beating the
orchestrator (**83%**, **17:57**) on both quality and latency — see below.*
