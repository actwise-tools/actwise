...
(``ACTONE_DB_URL`` / ``ACTONE_DB_*``) > named profile in ``actwise-data.yaml`` >
...
the gitignored ``actwise-data.secrets.yaml`` (``{profile: password}``) or a
...
from actwise.paths import find_config_compat, getenv_compat

DEFAULT_PROFILE_PATH = find_config_compat("actwise-data.yaml", "actone-data.yaml")
# Per-profile passwords live here (gitignored); never in the profile YAML.
DEFAULT_SECRETS_PATH = find_config_compat("actwise-data.secrets.yaml", "actone-data.secrets.yaml")
...
class ConnConfig:
    ...
    # Deployment schema prefix (e.g. "bppr_"). Solutions install into
    # ``<prefix><canonical>`` schemas (bppr_star_app). When None it is derived
    # from the platform ``schema`` (bppr_rcm -> "bppr_", actone -> "").
    ...
    def deployment_prefix(self) -> str:
        """Effective deployment prefix (explicit override or derived from schema)."""
        if self.schema_prefix is not None:
            return self.schema_prefix
        return derive_prefix(self.schema)

    def solution_schema(self, canonical: str) -> str:
        """Deployment schema for a solution's canonical schema.

        e.g. prefix ``bppr_`` + canonical ``star_app`` -> ``bppr_star_app``.

        Solutions that install into the platform base schema itself (no own
        schema — e.g. QAS, whose pack declares canonical ``rcm``) resolve to the
        configured platform ``schema`` directly, regardless of how that base is
        named at this deployment (``rcm`` / ``actone`` / ``bppr_rcm``).
        """
        canonical = (canonical or "").strip().lower()
        if canonical in _CANONICAL_BASES:
            return self.schema
        return f"{self.deployment_prefix}{canonical}"


# Canonical platform base schemas whose deployment prefix is everything before
# them (bppr_rcm -> "bppr_"). Ordered longest-first for greedy suffix match.
_CANONICAL_BASES = ("actone", "rcm", "udm", "idb")


def derive_prefix(schema: str) -> str:
    """Derive the deployment prefix from a platform base schema.

    ``bppr_rcm`` -> ``bppr_``; ``actone`` -> ``""``. Falls back to stripping the
    final ``_``-delimited token when no known canonical base matches.
    """
    s = (schema or "").strip().lower()
    if not s:
        return ""
    for base in sorted(_CANONICAL_BASES, key=len, reverse=True):
        if s == base:
            return ""
        if s.endswith("_" + base):
            return s[: -len(base)]
    if "_" in s:
        return s.rsplit("_", 1)[0] + "_"
    return ""
...
def list_profiles(path: Path | None = None, secrets_path: Path | None = None) -> list[dict]:
    ...
    default = getenv_compat("ACTWISE_DATA_PROFILE", "ACTONE_DATA_PROFILE") or "local"
...
def resolve(profile: str | None = "local", path: Path | None = None, **overrides) -> ConnConfig:
    ...
    if profile and profile != "local":
        ...
        if prof.get("schema_prefix") is not None:
            schema_prefix = str(prof["schema_prefix"])
        if prof.get("dsn"):
            dsn = prof["dsn"]
        ...
        if profiles["local"].get("schema_prefix") is not None:
            schema_prefix = str(profiles["local"]["schema_prefix"])
    ...
    if os.getenv("ACTONE_DB_SCHEMA_PREFIX") is not None:
        schema_prefix = os.environ["ACTONE_DB_SCHEMA_PREFIX"]
    _timeout_env = getenv_compat("ACTWISE_DATA_TIMEOUT_MS", "ACTONE_DATA_TIMEOUT_MS")
    if _timeout_env:
        timeout_ms = int(_timeout_env)
    ...
    if overrides.get("schema_prefix") is not None:
        schema_prefix = overrides["schema_prefix"]
    ...
    return ConnConfig(dsn=dsn, timeout_ms=timeout_ms, schema_prefix=schema_prefix, **values)
