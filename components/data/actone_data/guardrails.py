...
class Constraints:
    allowlist_prefixes: tuple[str, ...] = (_VIEW_PREFIX,)
...
def validate(
    ...
    prefixes = tuple(p.lower() for p in constraints.allowlist_prefixes)
    # ``*`` = wildcard: skip the coarse prefix pre-filter and rely solely on the
    # explicit ``allowed_views`` allowlist (+ the schema check). Used by solution
    # rule packs whose tables share no common prefix (e.g. SAM).
    wildcard = "*" in prefixes
    concrete_prefixes = tuple(p for p in prefixes if p != "*")
    deny = {d.lower() for d in constraints.deny_objects}
    ...
    for t in root.find_all(exp.Table):
        ...
        if t.db and t.db.lower() != schema_l:
            ...
            continue
        candidates = {name}
        table_schema = (t.db or schema_l or "").lower()
        if table_schema:
            candidates.add(f"{table_schema}.{name}")
        if deny.intersection(candidates):
            errors.append(f"table {t.name!r} is denied by the rule pack")
            continue
        if not wildcard and not any(name.startswith(prefix) for prefix in concrete_prefixes):
            if prefixes == (_VIEW_PREFIX,):
                errors.append(f"table {t.name!r} is not an allowlisted {_VIEW_PREFIX}* view")
            else:
                pretty = ", ".join(f"{p}*" for p in concrete_prefixes)
                errors.append(f"table {t.name!r} does not match allowed prefixes: {pretty}")
            continue
    ...
    if not real_tables:
        if prefixes == (_VIEW_PREFIX,):
            ...
            errors.append("query must read from at least one allowlisted object")
        else:
            pretty = ", ".join(f"{p}*" for p in concrete_prefixes)
            errors.append(f"query must read from at least one allowed object ({pretty})")
    ...
    effective_cap = min(int(cap), int(constraints.row_cap))
    effective = max(1, min(int(max_rows), effective_cap))
