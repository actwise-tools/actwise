...
# Per-check statement-timeout ceiling for the rolled-up ``perf_report`` so that no
# single catalog scan can push the aggregate past the MCP HTTP/SSE gateway window.
# Individual perf tools still use the full configured ``timeout_ms``.
_REPORT_CHECK_TIMEOUT_MS = 8000
...
def perf_report(cfg: "ConnConfig", solution: str | None = None) -> dict:
    ...
    # Bound each rolled-up check to a short statement timeout so a single slow
    # catalog scan (e.g. an unscoped unused-index scan on a large multi-schema DB)
    # cannot drag the whole report past the MCP HTTP/SSE gateway window. A check
    # that times out is recorded as a failed section and the report still returns.
    if is_dataclass(cfg) and hasattr(cfg, "timeout_ms"):
        cfg_fast = replace(cfg, timeout_ms=min(int(cfg.timeout_ms), _REPORT_CHECK_TIMEOUT_MS))
    else:
        cfg_fast = cfg

    # Each check opens its own short-lived read-only connection, so they are
    # independent and safe to run concurrently. Running them in parallel keeps the
    # whole report within the gateway timeout on large, loaded DBs (sequential
    # execution summed 7 catalog scans and could exceed ~25s).
    checks = {
        "health": lambda: health_check(cfg_fast),
        "indexes": lambda: index_issues(cfg_fast, schema=schema),
        "missing_indexes": lambda: missing_indexes(cfg_fast, schema=schema),
        "index_candidates": lambda: unindexed_foreign_keys(cfg_fast, schema=schema),
        "vacuum": lambda: vacuum_health(cfg_fast, schema=schema),
        "config": lambda: config_review(cfg_fast),
        "top_queries": lambda: top_queries(cfg_fast),
    }
    ...
    with ThreadPoolExecutor(max_workers=len(checks)) as pool:
        for name, result in pool.map(lambda item: _run(*item), checks.items()):
