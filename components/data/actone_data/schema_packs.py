...
def list_solutions() -> dict:
    """Enumerate bundled schema packs (latest version per solution) with coverage.

    Returns the ActOne platform (view-based) plus every solution pack, each with
    its grounding surface, schema, version, object counts, description coverage,
    and draft status — the discovery entry point for multi-solution grounding.
    """
    solutions = []
    for key in ["actone", *available_solutions()]:
        pack = load(key) if key != "actone" else _load_actone_pack()
        if pack is None:
            continue
        if pack.get("views") is not None:  # ActOne platform: v_acm_* views
            src = pack.get("source") or {}
            solutions.append({
                "solution": key,
                "surface": "platform",
                "schema": pack.get("schema"),
                "version": (src.get("db_product_version") or "").lstrip("Vv") or None,
                "object_kind": "views",
                "object_count": len(pack.get("views") or {}),
                "described_count": None,
                "draft": bool(pack.get("draft")),
                "draft_reason": pack.get("draft_reason"),
            })
        else:  # solution pack: tables
            s = table_summary(pack)
            solutions.append({
                "solution": key,
                "surface": "solution",
                "schema": s["schema"],
                "version": s["version"],
                "object_kind": "tables",
                "object_count": s["table_count"],
                "described_count": s["described_count"],
                "draft": s["draft"],
                "draft_reason": s["draft_reason"],
            })
    ready = sum(1 for s in solutions if not s["draft"])
    return {"count": len(solutions), "ready": ready, "solutions": solutions}


def _load_actone_pack() -> dict | None:
    """Load the latest ActOne platform (view-based) pack directly."""
    packs = sorted(DEFAULT_PACK_DIR.glob("schema-pack-actone-*.json"), key=_version_key)
    if not packs:
        return None
    try:
