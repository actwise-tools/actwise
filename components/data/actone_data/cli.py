...
def ping(
    profile: str = typer.Option("local", "--profile", "-p", help="Named profile from actwise-data.yaml (default: built-in local)."),
...
solutions_app = typer.Typer(
    add_completion=False,
    no_args_is_help=True,
    help="Business solutions: a product's licensed fraud lines (e.g. IFM Retail/Card/NAF) + client detection.",
)
app.add_typer(solutions_app, name="solutions")


@solutions_app.command("list", help="List a product's business solutions + add-ons and their DB markers.")
def solutions_list(
    product: str = typer.Option("ifm", "--product", "-P", help="Product key (default: ifm)."),
    fmt: str = typer.Option("table", "--format", "-f", help="table | json."),
):
    from actone_data import business_solutions

    result = business_solutions.list_business_solutions(product)
    if fmt == "json":
        import json as _json
        print(_json.dumps(result, indent=2))
        return
    if not result.get("found"):
        avail = ", ".join(result.get("available_products", [])) or "-"
        console.print(f"[yellow]no business-solution catalog for {product!r}[/yellow] (available: {avail})")
        raise typer.Exit(1)
    table = Table(box=None, pad_edge=False)
    table.add_column("key", style="cyan", no_wrap=True)
    table.add_column("official name")
    table.add_column("license flag")
    table.add_column("installer")
    table.add_column("markers", justify="right")
    for r in result["business_solutions"]:
        table.add_row(r["key"], r["official_name"] or "-", r["license_flag"] or "-",
                      "separate" if r["separate_installer"] else "shared", str(len(r["markers"])))
    for r in result["add_ons"]:
        table.add_row(r["key"], (r["official_name"] or "-") + " [add-on]", r["license_flag"] or "-",
                      "-", str(len(r["markers"])))
    console.print(table)
    console.print(f"[bold]{len(result['business_solutions'])}[/bold] business solutions, "
                  f"[bold]{len(result['add_ons'])}[/bold] add-ons for {result['product']} {result.get('version') or ''}")


@solutions_app.command("detect", help="Detect which business solutions a live deployment runs (introspects the DB).")
def solutions_detect(
    ...
    try:
        result = perf.detect_business_solutions(cfg, product=product)
    ...
    if not result.get("ok"):
        console.print(f"[red]detect failed[/red]: {result.get('errors')}")
        raise typer.Exit(1)
    table = Table(box=None, pad_edge=False)
    table.add_column("business solution", style="cyan")
    table.add_column("verdict")
    table.add_column("matched", justify="right")
    for r in result["present"]:
        table.add_row(r["official_name"] or r["key"], "[green]present[/green]", str(r.get("match_count", 0)))
    for r in result["absent"]:
        table.add_row(r["official_name"] or r["key"], "[dim]absent[/dim]", "0")
    for r in result["config_only"]:
        table.add_row(r["official_name"] or r["key"], "[yellow]config-only[/yellow]", "-")
    console.print(table)
    console.print(f"scanned {len(result['scanned_schemas'])} schema(s), {result['live_object_count']} live objects")
...
@schema_app.command("solutions", help="List all grounded surfaces: the ActOne platform plus each solution pack.")
def schema_solutions(
    fmt: str = typer.Option("table", "--format", "-f", help="Output format: table | json."),
):
    from actone_data import schema_packs

    result = schema_packs.list_solutions()
    if fmt == "json":
        import json as _json
        print(_json.dumps(result, indent=2))
        return
    table = Table(box=None, pad_edge=False)
    table.add_column("solution", style="cyan", no_wrap=True)
    ...
    for s in result["solutions"]:
        described = "-" if s["described_count"] is None else str(s["described_count"])
        status = "[yellow]draft[/yellow]" if s["draft"] else "[green]ready[/green]"
        table.add_row(
            s["solution"], s["surface"], str(s["schema"]), str(s["version"] or "-"),
            f"{s['object_count']} {s['object_kind']}", described, status,
        )
    console.print(table)
    console.print(f"[bold]{result['count']}[/bold] surfaces ({result['ready']} ready)")
...
def schema_show(
    ...
    pack: str = typer.Option(None, "--pack", help="Pack path (default: ACTWISE_DATA_PACK or bundled)."),
