...
| `actone-data query validate "<sql>"` | Dry-run the guardrail pipeline on a SQL string (no execution). `--solution <s>` to target a solution schema. |
| `actone-data query run "<sql>"` | Validate **and** execute a read-only SELECT; `--format table\|json\|csv`, `--question` for audit, `--solution <s>` for a solution schema. |
...
| `actone-data perf report` | Scored read-only performance report (rolls up all checks); `--format markdown\|json`. |
| `actone-data perf <check>` | Individual read-only diagnostics: `health`, `top-queries`, `indexes`, `missing-indexes`, `vacuum`, `bloat`, `config`, `extensions`, `explain "<sql>"`. |
...
# Multi-solution: query an installed solution's own schema (STAR/SAM/CDD/WLF/IFM)
actone-data query run "SELECT e_status, count(*) FROM star_form_parsing_details GROUP BY 1" --solution star -p popular-qas-dev
```

### Multi-solution query surface

Beyond the ActOne `v_acm_*` views, the engine can ground + guardrail queries against an
installed **solution's own schema**. `--solution <name>` (CLI) / `solution` (MCP tools)
selects a solution's *schema pack* (the allowlisted tables) and *rule pack* (grounding +
enforced constraints). The engine resolves the deployment-prefixed schema automatically
(canonical `star_app` → `bppr_star_app` when the platform schema is `bppr_rcm`), and the
allowlist is the solution pack's tables **intersected with live objects** in that schema.
Bundled solutions: `star`, `sam`, `cdd`, `wlf`, `ifm` (see `actone_data/data/schema-pack-*.json`
+ `rules-*.yaml`). Omit `--solution` (default `actone`) for the `v_acm_*` view surface.
```
...
**Solution packs.** Alongside the ActOne pack, `actone_data/data/` holds per-solution
schema packs (`schema-pack-<solution>-<ver>.json`, package-derived table inventories) and
rule packs (`rules-<solution>-<ver>.yaml`, human-editable grounding + enforced
constraints). These drive `--solution` queries (see above). Rule packs layer over the
ActOne base pack and are **tightening-only**. The MCP *grounding* tools
(`get_schema_summary` / `list_views` / `describe_view`) and the CLI `schema` commands
accept a `solution` argument too, so the model can discover a solution's tables/columns
before querying them (default `actone` = the `v_acm_*` view surface).
