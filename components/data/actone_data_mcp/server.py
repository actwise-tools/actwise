...
Run (Streamable HTTP, for containers / remote clients / Copilot Studio):
    ...
    # optional shared secret: set ACTWISE_DATA_PROXY_API_KEY (header X-API-Key).

Connection: resolved from ACTWISE_DATA_PROFILE (default ``local``) / ACTWISE_DATA_DSN
/ ACTONE_DB_* env / the ``actwise-data.yaml`` profiles, exactly like the CLI.
(Legacy ACTONE_DATA_* env vars and actone-data.yaml are still honored as a fallback.)
"""
...
from actwise.paths import getenv_compat
...
mcp = FastMCP(
    SERVER_NAME,
    stateless_http=True,
    # Return each tool response as a single application/json HTTP response instead
    # of a long-lived SSE stream. A tool call is request/response (not a streaming
    # notification), and SSE streams are fragile through Cloudflare tunnels /
    # SSL-inspection proxies (observed as "SSE stream ended ... frames=0" and
    # intermittent 502/520). JSON responses traverse those hops reliably.
    json_response=True,
...
def _default_env() -> str:
    return getenv_compat("ACTWISE_DATA_PROFILE", "ACTONE_DATA_PROFILE") or "local"


def _cfg(env: str = ""):
    """Resolve the DB config for the requested environment (profile), cached per env.

    ``env`` is a profile name from ``actwise-data.yaml`` (see ``list_environments``);
    empty selects the default (``ACTWISE_DATA_PROFILE`` or ``local``). Returns
    ``(env_name, ConnConfig)``.
    """
    ...
    with _lock:
        if name not in _cfg_cache:
            # ACTWISE_DATA_DSN (legacy ACTONE_DATA_DSN) is a single global override -> only for the default env.
            dsn = getenv_compat("ACTWISE_DATA_DSN", "ACTONE_DATA_DSN") if name == _default_env() else None
...
#   py -m uvicorn actone_data_mcp.server:app --host 0.0.0.0 --port 8766
#   endpoint: http://localhost:8766/mcp   health: http://localhost:8766/healthz
app = _AuthGate(mcp.streamable_http_app(), getenv_compat("ACTWISE_DATA_PROXY_API_KEY", API_KEY_ENV))
