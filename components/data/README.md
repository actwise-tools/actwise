...
CDD Profiles, CDD DART, STAR add their own). Each is modeled as a `solution`
schema pack + rule pack (query conventions + PII masking); the deployed schema
name (e.g. `bppr_cdd_prf`) is resolved from the environment prefix. A second,
...
| `schema` | Introspect the live `v_acm_*` views / build / show / summarize the schema pack (`--solution` for a solution pack). |
...
`actone-data-mcp` exposes **17 read-only tools** across two surfaces.
...
| `perf_report` | Scored recommendation report — findings carry severity, evidence, fix, `source_url`. Accepts `solution`. |
