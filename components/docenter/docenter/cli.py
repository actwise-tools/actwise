...
def auth_connect(
    broker: str = typer.Option(
        ..., "--broker", "-b", help="Broker base URL, e.g. https://broker.example.com"
    ),
    state: str = typer.Option(
        ..., "--state", "-s", help="One-time login state minted by the portal (POST /links)"
    ),
    url: str = typer.Option(
        "", "--url", "-u", help="Override portal URL (defaults to DOCENTER_PORTAL_URL)"
    ),
):
    """Capture THIS device's DOCenter SSO session and send it to the portal broker.

    For NICE employees on a managed device: opens a browser for SSO (Conditional
    Access passes on your own device), captures your ``_SESSION``, and POSTs it to
    the broker's ``/ingest`` endpoint authorized by the one-time ``state``. The
    broker stores it under the user id embedded in the signed state (never a field
    you supply), so the hosted docs MCP can then answer with your own entitlements.
    """
    import requests

    target_url = url or PORTAL_URL
    all_cookies, _found = _browser_capture(
        site_url=target_url, detect_cookie="_SESSION", label="Zoomin", force_sso=True,
    )
    payload = cookies_to_payload(all_cookies)

    ingest_url = broker.rstrip("/") + "/ingest"
    rprint(f"[cyan]Sending your session to the broker...[/cyan] [dim]{ingest_url}[/dim]")
    try:
        resp = requests.post(ingest_url, json={"state": state, "payload": payload}, timeout=30)
    except Exception as exc:  # noqa: BLE001
        rprint(f"[red]Failed to reach broker:[/red] {exc}")
        raise typer.Exit(1)
    ...
    if resp.status_code != 200:
        ...
        try:
            ...
            pass
        rprint(f"[red]Broker rejected the session (HTTP {resp.status_code}):[/red] {detail}")
        raise typer.Exit(1)

    body = resp.json()
    rprint(f"[green]Connected![/green] Your DOCenter session is now linked "
           f"(user: {body.get('user_id', '?')}).")
    rprint("[dim]Return to the portal / chat and ask your question again.[/dim]")
...
def _browser_capture(
    ...
    """Open a browser, let the user complete login, and return ``(all_cookies, found)``.

    The shared browser-capture core: a persistent context (preserves the full
    SSO cookie chain across Microsoft → Zoomin → portal redirects), optional
    credential auto-fill, and the ``zoomin_login_complete`` success signal. Both
    ``_browser_login`` (writes a file) and ``auth connect`` (POSTs to the broker)
    build on this so there is exactly one capture path. Raises ``typer.Exit`` on
    a missing browser dep or a login timeout."""
    try:
...
def _browser_login(
    ...
    """
    all_cookies, found = _browser_capture(
        site_url=site_url,
        detect_cookie=detect_cookie,
        label=label,
        email=email,
        password=password,
        force_sso=force_sso,
