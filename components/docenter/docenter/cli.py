...
def _browser_login(
    ...
    with sync_playwright() as p:
        ...
        while time.time() < deadline:
            ...
            # For Zoomin portal: require ZD__userAuthenticated = "true" AND _SESSION present.
            # _SESSION is httpOnly on docs-be.niceactimize.com — it arrives slightly after
            # ZD__userAuthenticated, so we must wait for both.
            if auth_cookie and str(auth_cookie.get("value", "")).lower() == "true" and session_cookie:
                found = session_cookie
                break
            # Fallback for other services (SharePoint etc.) that don't use ZD__userAuthenticated
            if session_cookie and detect_cookie != "_SESSION" and "/auth/login" not in page.url:
                found = session_cookie
                break
            time.sleep(2)
