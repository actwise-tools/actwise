...
def fetch_pdf_text(
    ...
    """Download a bundle *resource* PDF and extract its text.

    Older / archived bundles (e.g. ``Actimize_ActOne_6.4.1_Installation_Guide``)
    expose their content only as ``/bundle/<bundle>/resource/<file>.pdf`` instead
    of HTML ``/page/`` topics. The backend ``/api/.../resource/<file>`` endpoint
    returns a JSON descriptor whose ``topic_url`` is the raw PDF download link;
    we resolve that, fetch the bytes, and extract text. Returns
    ``{title, text, page_count, truncated}``. ``resource_path`` is the file
    segment after ``/resource/`` (e.g. ``Foo.pdf``).
    """
    from io import BytesIO

    from pypdf import PdfReader

    # 1) Resolve the raw download URL from the resource descriptor.
    meta_url = f"{BASE_API}/bundle/{bundle}/resource/{resource_path}"
    meta_resp = session.get(meta_url, timeout=30)
    meta_resp.raise_for_status()
    descriptor = meta_resp.json()
    raw_url = descriptor.get("topic_url") or (
        f"https://docs-be.niceactimize.com/bundle/{bundle}/raw/resource/enus/{resource_path}"
    )
    title = descriptor.get("bundle_title") or ""

    # 2) Download the PDF bytes and extract text.
    pdf_resp = session.get(raw_url, timeout=60)
    ...
    if not title:
        ...
        except Exception:  # pypdf raises on some malformed metadata
            title = ""
    return {
        "title": title,
        "text": text,
