"""Signed ``X-DOCenter-User`` token — MCP-side verifier (Phase 3, R2).

Byte-for-byte compatible with the TypeScript signer the eve portal uses to mint
the header (``components/portal/eve/agent/lib/docenter-user-token.ts``) and with
its Python fixture twin. Promoted here from that fixture so the MCP's
``_AuthGate`` can *verify and enforce* the signature (R2): any holder of the
shared ``X-API-Key`` must ALSO present a signature-valid, unexpired user token to
act as a specific end user.
...
def mint_docenter_user_token(
    ...
    """Mint a token (used by tests / seeding; the portal mints in TypeScript)."""
    if not user_id:
...
def verify_docenter_user_token(
    ...
    """Verify a token's signature + expiry; raise ``ValueError`` on any failure."""
    parts = token.split(".")
    if len(parts) != 4:
