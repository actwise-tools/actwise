...
from extractor.extractor import (
    fetch_page,
    fetch_pdf_text,
    get_toc as _extractor_get_toc,
    html_to_markdown,
)

# Phase 3 (per-user identity) — verify the signed X-DOCenter-User header and map a
# verified user to their own portal cookie. Both are inert unless DOCENTER_PER_USER
# is set, so the flag-off image is byte-for-byte the shared-cookie server.
from docenter_mcp.user_token import verify_docenter_user_token
from docenter_mcp.user_store import load_user_cookie_data

SERVER_NAME = "actwise-docenter-live"
API_KEY_ENV = "DOCENTER_PROXY_API_KEY"

# ── Per-user identity (Phase 3, additive + flag-gated) ───────────────────────
# When DOCENTER_PER_USER is on, a signature-valid X-DOCenter-User header binds a
# request to a specific end user and routes it to that user's own portal cookie.
# When off (the Copilot agent's image), none of this code path runs.
DOCENTER_PER_USER = os.environ.get("DOCENTER_PER_USER", "").strip().lower() in (
    "1", "true", "yes", "on",
)
USER_TOKEN_SECRET_ENV = "DOCENTER_USER_TOKEN_SECRET"

# ── Login broker (Phase 4, optional) ─────────────────────────────────────────
# When DOCENTER_BROKER_URL is set, a SessionRequired is enriched with a one-time
# login_url minted by the broker (POST /links, authenticated with the shared
# DOCENTER_BROKER_SECRET — R1′). Unset → plain SessionRequired (Phase-3 behavior).
DOCENTER_BROKER_URL_ENV = "DOCENTER_BROKER_URL"
DOCENTER_BROKER_SECRET_ENV = "DOCENTER_BROKER_SECRET"
...
def _get_session():
    ...
    ``load_session`` raises ``typer.Exit(1)`` when the cookie file is missing or
    invalid. Depending on the installed Typer/Click version that class is either a
    ``SystemExit`` or a plain ``RuntimeError`` (Typer 0.12+/Click 8.1 make it a
    ``RuntimeError``), so catch both — otherwise the exit leaks out as a bare "1"
    and the cold-start bootstrap in ``_run_portal`` never sees ``PortalUnavailable``."""
    global _session
    with _session_lock:
        if _session is None:
            try:
                _session = load_session()
            except (SystemExit, typer.Exit) as exc:  # missing/invalid cookie file
...
class SessionRequired(PortalUnavailable):
    """The authenticated end user has no valid portal session of their own.

    Subclass of ``PortalUnavailable`` so existing handlers treat it as
    unavailable, but distinct so the Phase 4 login broker can attach a login link.
    Raised only on the per-user path; NEVER falls back to the shared cookie (R3)."""

    def __init__(self, user_id: str, login_url: Optional[str] = None):
        self.user_id = user_id
        self.login_url = login_url
        msg = f"No DOCenter session for user '{user_id}'. Interactive login is required"
        if login_url:
            msg += f" — open this link to connect your DOCenter account: {login_url}"
        else:
            msg += " (the login broker will provide a one-time login link)."
        super().__init__(msg)


def _mint_login_url(user_id: str) -> Optional[str]:
    """Ask the broker for a one-time login link (best-effort). None if unconfigured/down."""
    base = os.environ.get(DOCENTER_BROKER_URL_ENV, "").strip().rstrip("/")
    secret = os.environ.get(DOCENTER_BROKER_SECRET_ENV, "")
    ...
    except Exception:  # noqa: BLE001 — broker down ⇒ plain SessionRequired, never crash
        return None
    return None


def _make_session_required(user_id: str) -> SessionRequired:
    """Build a SessionRequired, minting a broker login_url when configured.

    Must be called OUTSIDE the per-user session lock (it may do broker I/O)."""
    return SessionRequired(user_id, _mint_login_url(user_id))


# ── Per-user portal sessions (Phase 3, isolated from the shared session) ─────
_user_session_lock = threading.Lock()
_user_sessions: dict = {}  # user_id -> requests.Session (per-user cookie)


def _get_user_session(user_id: str):
    """Return the cached per-user session, building it from the user store.

    Raises ``SessionRequired`` when the user has no seeded cookie — this path
    never touches the shared cookie file, keeping the two identities provably
    isolated (R3)."""
    with _user_session_lock:
        sess = _user_sessions.get(user_id)
        if sess is not None:
            return sess
        data = load_user_cookie_data(user_id)
        if data is not None:
            sess = build_session_from_cookies(data)
            _user_sessions[user_id] = sess
            return sess
    # No stored cookie for this user — release the lock before any broker I/O,
    # then raise a SessionRequired (optionally carrying a broker login link).
    raise _make_session_required(user_id)


def _drop_user_session(user_id: str) -> None:
    """Drop a cached per-user session (e.g. after that user's cookie expired)."""
    with _user_session_lock:
        _user_sessions.pop(user_id, None)


def _current_user_id() -> Optional[str]:
    """Read the (already-verified) X-DOCenter-User subject from the request context.

    Per R2′: the streamable-HTTP transport threads the Starlette request into the
    MCP request context in the SAME task that runs the tool, so ``get_context()``
    is the officially-supported way to read the header here. ``_AuthGate`` has
    already HMAC-verified it and 401'd bad tokens; re-verifying is cheap and keeps
    this function safe on its own. Returns None when no user header is present
    (the shared-cookie / Copilot-agent path)."""
    secret = os.environ.get(USER_TOKEN_SECRET_ENV, "")
    try:
        request = mcp.get_context().request_context.request
        raw = request.headers.get("x-docenter-user") if request is not None else None
    except Exception:  # noqa: BLE001 — no request context ⇒ treat as no user header
        raw = None
    if not raw:
        return None
    try:
        return verify_docenter_user_token(raw, secret).user_id
    except ValueError as exc:  # unreachable if _AuthGate ran, but never serve shared
        raise PortalUnavailable("Invalid X-DOCenter-User token.") from exc


def _resolve_session():
    """Return ``(session, user_id)`` for the current request.

    ``user_id`` is None on the shared-cookie path. Resolution:
      * DOCENTER_PER_USER off        → shared session (byte-for-byte today).
      * flag on, no user header      → shared session (Copilot agent path).
      * flag on, user header present → that user's own session, or SessionRequired
                                       when the user has no seeded cookie."""
    if not DOCENTER_PER_USER:
        return _get_session(), None
    user_id = _current_user_id()
    if user_id is None:
        return _get_session(), None
    return _get_user_session(user_id), user_id
...
def _run_portal(fn, *args, **kwargs):
    ...
    On the per-user path a 403 raises ``SessionRequired`` instead of triggering the
    shared HTTP re-login — the shared cookie is never written on a user's behalf (R3)."""
    try:
        session, user_id = _resolve_session()
    except SessionRequired:
        raise  # per-user path — never bootstrap the shared cookie on a user's behalf
    except PortalUnavailable:
        # Cold start: no shared cookie seeded yet. Treat a missing cookie like an
        # expired one — mint a fresh cookie from DOCENTER_EMAIL / DOCENTER_PASSWORD via
        # the same throttled browser-free HTTP login used for self-heal, then retry.
        if not _try_relogin():
            raise
        session, user_id = _resolve_session()
    try:
        ...
        if user_id is not None:
            # Per-user cookie expired: drop it and require an interactive re-login.
            # Do NOT touch the shared cookie or the shared re-login throttle (R3).
            _drop_user_session(user_id)
            raise _make_session_required(user_id) from exc
        # Shared path: try one automatic browser-free re-login from env creds.
        if _try_relogin():
            try:
...
_PORTAL_PDF_RE = re.compile(r"/bundle/([^/]+)/resource/(.+?\.pdf)$", re.IGNORECASE)
...
def _parse_portal_pdf_url(url: str):
    """Split a resource-PDF citation URL into (bundle, resource_path).

    Older / archived bundles expose content as ``/bundle/<bundle>/resource/
    <file>.pdf`` instead of HTML ``/page/`` topics. Returns ``(None, None)`` when
    the URL isn't a recognizable resource-PDF link."""
    cleaned = url.split("#", 1)[0].split("?", 1)[0]
    m = _PORTAL_PDF_RE.search(cleaned)
    if not m:
        return None, None
    return m.group(1), m.group(2)
...
def _fetch_pdf_core(session, bundle: str, resource_path: str, max_chars: int) -> dict:
    """``fetch_pdf_text`` wrapper mirroring ``_fetch_page_core``'s 403 mapping."""
    try:
        return fetch_pdf_text(session, bundle, resource_path, max_chars=max_chars)
    except requests.HTTPError as exc:
        if exc.response is not None and exc.response.status_code == 403:
            raise RuntimeError("access_denied") from exc
        raise RuntimeError(str(exc)) from exc
...
def get_page(url: str, max_chars: int = 12000) -> dict:
    ...
    search snippet, so answers can quote the source accurately. Both HTML topic
    pages (`.../page/...`) and older/archived **resource PDFs**
    (`.../resource/<file>.pdf`) are supported; PDF pages set `format: "pdf"`.
    ...
    Args:
        url: A page citation URL (the `portal_url` from `search_docs`), e.g.
            ...
            or a resource PDF "https://docs.niceactimize.com/bundle/<bundle>/resource/<file>.pdf".
    ...
    Returns:
        ...
        `truncated` (and `format` for PDFs) — or an `error` ("bad_url" /
        "not_found") when the page can't be resolved.
    """
    ...
    # Fall back to resource-PDF handling (older / archived bundles).
    pdf_bundle, resource_path = _parse_portal_pdf_url(url)
    if pdf_bundle:
        data = _run_portal(_fetch_pdf_core, pdf_bundle, resource_path, max_chars)
        text = (data.get("text") or "").strip()
        if not text:
            return {
                "error": "not_found",
                "message": (
                    f"No extractable text in {pdf_bundle}/{resource_path} "
                    "(the PDF may be scanned/image-only)."
                ),
                "bundle": pdf_bundle,
                "format": "pdf",
            }
        truncated = data.get("truncated") or len(text) > max_chars
        return {
            "title": data.get("title") or pdf_bundle.replace("_", " "),
            "bundle": pdf_bundle,
            "version": extract_version(pdf_bundle),
            "updated": "",
            "url": f"https://docs.niceactimize.com/bundle/{pdf_bundle}/resource/{resource_path}",
    ...
    return {
        ...
        "or a resource PDF .../bundle/<bundle>/resource/<file>.pdf",
    }
    ...
    pages = [
        {
            "title": e.get("title", "-"),
            "portal_url": (
                f"https://docs.niceactimize.com/bundle/{bundle}/resource/{e['path']}"
                if str(e.get("path", "")).lower().endswith(".pdf")
                else f"https://docs.niceactimize.com/bundle/{bundle}/page/{e['path']}"
            ),
        }
...
class _AuthGate:
    ...
    and (when DOCENTER_PER_USER is on) HMAC-verifies the X-DOCenter-User header.
    ...
    def __init__(self, app, api_key: Optional[str], per_user: bool = False,
    ...
    async def __call__(self, scope, receive, send):
        ...
        if self.api_key:
            ...
            if not (provided and hmac.compare_digest(provided, self.api_key)):
                ...
                return
        # Per-user identity (R2): a present X-DOCenter-User must be signature-valid
        # and unexpired, else 401 — so an X-API-Key holder can't impersonate a user.
        # An absent header is allowed (shared-cookie path).
        if self.per_user:
            raw = headers.get(b"x-docenter-user", b"").decode()
            if raw:
                if not self.user_token_secret:
                    await JSONResponse(
                        {"error": "server_misconfigured"}, status_code=500,
                    )(scope, receive, send)
                    return
                try:
                    verify_docenter_user_token(raw, self.user_token_secret)
                except ValueError:
                    await JSONResponse(
                        {"error": "invalid_user_token"}, status_code=401,
                    )(scope, receive, send)
                    return
        await self.app(scope, receive, send)


# ── Optional embedded login broker (Option-2 PoC topology, 2026-07-18) ─────────
# For the PoC the docs MCP and the login broker share ONE App Runner service:
# broker paths are dispatched to the broker's FastAPI app (which authenticates via
# the signed one-time `state` / broker secret, NOT the MCP X-API-Key), everything
# else stays behind the _AuthGate → MCP. Activated only when DOCENTER_BROKER_SECRET
# is set, so a shared-only deployment (no broker secret) is byte-for-byte the plain
# gated MCP. FUTURE (Option 1): split the broker into its own App Runner service
# with a write-only S3 role — see docs/components/portal/2026-07-17-per-user-portal-as-built.md.
class _EmbeddedBroker:
    """Route broker paths to the broker app; everything else to the gated MCP.

    Pure ASGI: HTTP requests whose path is a broker route go to ``broker_app``
    (bypassing the API-key gate, as intended for the browser/CLI login flow); all
    other HTTP plus every lifespan/websocket event go to ``mcp_app`` so the FastMCP
    session manager's lifespan is driven exactly as before."""

    _BROKER_PREFIXES = ("/links", "/connect", "/ingest", "/status", "/password-login")

    def __init__(self, mcp_app, broker_app):
        self.mcp_app = mcp_app
        self.broker_app = broker_app

    def _is_broker_path(self, path: str) -> bool:
        return any(path == p or path.startswith(p + "/") for p in self._BROKER_PREFIXES)

    async def __call__(self, scope, receive, send):
        if scope.get("type") == "http" and self._is_broker_path(scope.get("path", "")):
            await self.broker_app(scope, receive, send)
            return
        await self.mcp_app(scope, receive, send)


def _build_app():
    """Build the ASGI app: the gated MCP, optionally fronted by the embedded broker.

    The embedded broker is enabled only when ``DOCENTER_BROKER_SECRET`` is set
    (the signal that this instance also hosts the login broker); otherwise the app
    is exactly the plain ``_AuthGate``-wrapped MCP (unchanged shared behavior)."""
    mcp_app = _AuthGate(
        mcp.streamable_http_app(),
        os.environ.get(API_KEY_ENV),
        per_user=DOCENTER_PER_USER,
        user_token_secret=os.environ.get(USER_TOKEN_SECRET_ENV, ""),
    )
    if os.environ.get("DOCENTER_BROKER_SECRET"):
        from docenter_broker.app import app as broker_app
        return _EmbeddedBroker(mcp_app, broker_app)
    return mcp_app
