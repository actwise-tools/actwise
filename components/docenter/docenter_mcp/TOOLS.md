...
For the end-to-end request flow (tool call → Zoomin API) with diagrams, see
[FLOWS.md](./FLOWS.md).
...
can quote the source accurately. Pass the result's `portal_url`. Both HTML topic
pages (`.../page/...`) and older/archived **resource PDFs** (`.../resource/<file>.pdf`)
are supported — PDF sources are downloaded and text-extracted, and flagged with
`format: "pdf"` in the result.

### Parameters

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `url` | string | **required** | A page citation URL (the `portal_url` from `search_docs`) — an HTML page `https://docs.niceactimize.com/bundle/<bundle>/page/Content/.../<Page>.htm` **or** a resource PDF `https://docs.niceactimize.com/bundle/<bundle>/resource/<file>.pdf`. |
...
```jsonc
{
  ...
  "truncated": false
  // resource-PDF results additionally carry:  "format": "pdf"
}
```
...
| `error` | When |
|---------|------|
| `bad_url` | The URL isn't a recognizable `.../bundle/<bundle>/page/<nav_path>` page link or `.../bundle/<bundle>/resource/<file>.pdf` resource link. |
| `not_found` | The page resolved but returned no content (HTML `topic_html` empty, or a scanned/image-only PDF with no extractable text). |
