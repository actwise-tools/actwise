...
**Request-flow diagrams** — every tool traced from the MCP call to the Zoomin API
endpoint: **[FLOWS.md](./FLOWS.md)**.
...
### Team / single-credential deploy

For teammates who want a **shared-cookie** deploy (one docenter credential, no per-user
identity, no S3, no login broker):

- **[team-deployment/](./team-deployment/)** — the self-contained, provider-agnostic
  **deployment kit** shipped in the public repo: a package README, a
  [DEPLOYMENT-GUIDE](./team-deployment/DEPLOYMENT-GUIDE.md) (build → registry → secrets →
  run on App Runner / ECS / k8s → verify), and a
  [USER-GUIDE](./team-deployment/USER-GUIDE.md) for connecting MCP clients.
- **[README-team-deploy.md](./README-team-deploy.md)** — how single-credential mode works,
  sharing the code, safe credential config, one-time AWS bootstrap, and client wiring.
- **[deploy-shared.ps1](./deploy-shared.ps1)** — thin wrapper over `deploy.ps1` that forces
  shared mode (2-4 secrets, no per-user/S3/broker env vars).
- **[HANDOFF-image-spec.md](./HANDOFF-image-spec.md)** — image-handoff spec for the
  Ansible/platform team to deploy the container into org AWS/VPC (env + secret contract,
  egress hosts, ECS/Kubernetes templates).
