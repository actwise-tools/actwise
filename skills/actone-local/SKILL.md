...
> **Development mode + logging (baked into `run`).** Two things make a local box debuggable:
> - **`actimize.mode = development`** — written into acm.ini by `render-config`/`encrypt-config`
>   when `config.dev_mode` is true (the default). In development mode ActOne returns **stack
>   traces and DB errors to the caller** instead of the generic *"Error occurred while processing
>   request… contact your System Administrator"* 500, and reloads Velocity/XSL templates per call.
>   Set `dev_mode: false` for production-like behaviour. Changing it needs a container restart.
>   (Ref: ActOne 10.2 Installation Guide, *Introduction to actimize.mode* / *Differences in
>   Behavior*.)
> - **A host-mounted log dir.** The image's `/usr/local/tomcat/bin/acm` is **root-owned (755)**
>   while the container runs as uid 1000 (`tomcat`), so ActOne can't create its `actimizeLogs`
>   dir — which silently breaks **every** file appender (no logs at all, and the console/memory
>   appenders are null too, so even startup errors vanish). `run` bind-mounts
>   **`{work_dir}/logs` → `/usr/local/tomcat/bin/acm/actimizeLogs`**, a host-writable dir, so
>   logging initialises and the logs (`actimizeErr.log`, `actimizeWarn.log`, `DARTQueries.log`, …)
>   appear on the host under `packages/actone-local/logs/`. Tail them there when a REST/SOAP call
>   fails.
...
`dev_mode=True` (acm.ini `actimize.mode=development` — caller-visible stack traces + host-mounted
logs), base image `amazoncorretto:21-al2023`, image `actone:10.2`, http port 8082.
...
| Generic `500 … contact your System Administrator` from a REST/SOAP call | ActOne is in **production** mode masking the real error. Ensure `dev_mode: true` (acm.ini `actimize.mode=development`) and restart; the response then carries the stack trace. Also tail `packages/actone-local/logs/actimizeErr.log`. |
| No logs anywhere (empty `actimizeLogs`, missing appenders at startup) | The `acm` dir is root-owned so the container can't create `actimizeLogs`. `run` bind-mounts `{work_dir}/logs` there to fix it — re-`run` (don't just `docker start`) so the mount is applied. |
