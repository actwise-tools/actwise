...
## Copilot Studio runtime profile (READ FIRST)

When this skill runs inside a Microsoft Copilot Studio agent (GitHub Copilot harness sandbox), the environment differs from a local CLI. Apply these overrides; the rest of this document still governs authoring.

- **Invocation & working directory.** Scripts run from this skill's root. Invoke the CLI as `node bin/archify.mjs <command> ...`, with input/output paths relative to the skill root or an absolute sandbox path.
- **No network at runtime.** Skip the "Update awareness" step entirely — `scripts/check-update.mjs` cannot reach the network. Do not run it and do not mention updates.
- **HTML is the only in-sandbox output.** `render`, `validate`, and `deliver` are pure Node and work offline. Do NOT run `visual-check`, `preview --open`, or any PNG/JPEG/WebP/SVG/WebM export — those need a browser that is not available. Produce the self-contained interactive `.html` with `deliver` and stop. Report the artifact path plus the deterministic validation/delivery receipt only; never claim browser evidence or visual inspection you did not perform.
- **Delivering the file to the user.** The sandbox cannot call connectors from inside a script. After `deliver` writes the `.html`, hand that file to the agent's own tools at the orchestrator layer: save it to the user's OneDrive via the connected OneDrive tool (for example into `/Downloads`) and/or return it as a downloadable file in the conversation.
- **Everything else is unchanged.** The fast authoring path (choose type → read one schema + example → author JSON → `validate --quality showcase` → `deliver`) applies exactly as written below.
