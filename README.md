<p align="center">
  <img src="./assets/readme/hero.svg" width="100%"
       alt="ActWise — an engineering toolkit for NICE Actimize ActOne: docs, live REST, read-only SQL, utilities, install media, and a local ActOne, installed as one tool.">
</p>

<p align="center">
  <a href="./LICENSE"><img alt="License: Apache-2.0" src="https://img.shields.io/badge/License-Apache_2.0-1f6feb.svg"></a>
  <img alt="Python ≥ 3.10" src="https://img.shields.io/badge/python-%E2%89%A5%203.10-1f6feb">
  <img alt="Status: beta" src="https://img.shields.io/badge/status-beta-d29922">
  <img alt="Unofficial — not a NICE product" src="https://img.shields.io/badge/unofficial-not%20a%20NICE%20product-8b949e">
  <a href="https://actwise-tools.github.io/actwise/"><img alt="Docs: wiki" src="https://img.shields.io/badge/docs-wiki-8250df"></a>
</p>
...
### Self-hosting `actone-mcp` / `actone-data-mcp`

These two reach a live ActOne instance/database, so they need a little setup:

- **Config:** copy `components/ops/actone-ops.example.yaml` and
  `components/data/actone-data.example.yaml` to `~/.actwise/` and fill in your
  instance URL / DB host (resolution order: `$ACTWISE_CONFIG_DIR` → cwd →
  `~/.actwise` → repo root). Profile YAML holds **no passwords**.
- **Secrets:** supply passwords via env — `ACTONE_PASSWORD__<ENV>` (ops) and
  `ACTONE_DB_PASSWORD__<PROFILE>` (data) — or a sibling `*.secrets.yaml`.
- **Ops first run:** the snapshot ships **no bundled REST spec**. Fetch one from
  your instance once — `actone fetch-spec --url http://<host>:8080/RCM --user <u>
  --password <p>` — before using `search_ops`/`describe_op`/`invoke_op`.
- **Safety:** ops writes are **default-deny** (opt in per env with `allow_writes`
  or globally with `ACTONE_ALLOW_WRITES`); data is **strictly read-only**.
- **Shared HTTP deploy:** run the ASGI app (`uvicorn actone_mcp.server:app` /
  `actone_data_mcp.server:app`) and set `ACTONE_PROXY_API_KEY` /
  `ACTONE_DATA_PROXY_API_KEY` to require an `X-API-Key` header on the tunnel.

See the wiki pages ([actone-mcp](https://actwise-tools.github.io/actwise/mcp/actone-mcp/),
[actone-data-mcp](https://actwise-tools.github.io/actwise/mcp/actone-data-mcp/)) for
the full setup.
...
ecosystem narrative, and an exhaustive auto-generated CLI reference — is published at:

**➜ <https://actwise-tools.github.io/actwise/>**

The wiki source lives under `components/wiki/` and builds with MkDocs Material:
